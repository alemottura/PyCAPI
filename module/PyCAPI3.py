...
class CanvasAPI():
	...
	def get(self, api, to_json=True, payload=None, single=False, first_page_only=False):
		...
		if single: # if request is meant to have returned a single item, just return the item
			if len(responses) > 1:
				for item in responses[0].keys():
					for i in range(1,len(responses)):
						responses[0][item].extend(responses[i][item])
				return responses[0]
			else:
				return responses[0]
	...
	def patch(self, api_url, payload=None):
		url = self.api_url + api_url
		if payload is None:
			payload = {}
		r = self.session.patch(url, data=payload)
		r.raise_for_status()
		return r
	...
	def get_assignment(self, course_id, assignment_ids, include=None, override_assignment_dates=False):
		"""Obtain a specific assignment in a specific course."""
		payload = {}
		payload['override_assignment_dates'] = override_assignment_dates
		if include is not None:
			payload['include[]'] = include
		return self.get('/courses/%s/assignments/%s' % (course_id, assignment_ids), payload=payload, single=True)
	
	def get_assignments(self, course_id, include=None, override_assignment_dates=False):
		"""Obtain assignments in a specific course."""
		payload = {}
		payload['override_assignment_dates'] = override_assignment_dates
		if include is not None:
			payload['include[]'] = include
		return self.get('/courses/%s/assignments' % course_id, payload=payload)
	...
	def get_assignment_submissions(self,course_id, assignment_ids='', user_ids='all', grouped=True):
		...
		if grouped == True:
			...
			return self.get('/courses/%s/students/submissions?grouped=false' % course_id, payload=payload)
	...
	def download_annotated_submission(self, course_id, assignment_id, user_id):
		...
		api_url = submission[0]['attachments'][0]['preview_url'][7:]
