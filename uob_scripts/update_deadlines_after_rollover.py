	...
	for assignment in assignments:
		if assignment['due_at'] == None:
			...
			if uob_utils.AcademicYear(datetime.datetime.strptime(assignment['due_at'], "%Y-%m-%dT%H:%M:%SZ").date()) == 2019:
				...
				proposed_due_date = uob_utils.FindCorrespondingDate(datetime.datetime.strptime(assignment['due_at'], "%Y-%m-%dT%H:%M:%SZ").date(),2019)
				...
				if change == 'y':
					if assignment['lock_at'] != None:
						proposed_lock_date = uob_utils.FindCorrespondingDate(datetime.datetime.strptime(assignment['lock_at'], "%Y-%m-%dT%H:%M:%SZ").date(),2019)
					...
					if assignment['unlock_at'] != None:
						proposed_unlock_date = uob_utils.FindCorrespondingDate(datetime.datetime.strptime(assignment['unlock_at'], "%Y-%m-%dT%H:%M:%SZ").date(),2019)
